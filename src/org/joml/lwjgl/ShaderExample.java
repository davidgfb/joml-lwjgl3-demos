...
public class ShaderExample {
	...
	void initOpenGLAndRenderInAnotherThread() {
		...
		glShaderSource(vs, "uniform mat4 viewProjMatrix;" + "void main(void) {" + "  gl_Position = viewProjMatrix * gl_Vertex;"
				+ "}");
		glCompileShader(vs);
		glAttachShader(program, vs);
		int fs = glCreateShader(GL_FRAGMENT_SHADER);
		glShaderSource(fs, "void main(void) {" + "  gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0);" + "}");
		glCompileShader(fs);
	...
	void loop() {
		/* Spawn a new thread which creates the OpenGL context and does the rendering. */
